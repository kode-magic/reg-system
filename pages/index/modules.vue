  ...
  <div>
    <div class="container mx-auto px-2 pt-2 pb-10">
      <div class="bg-white p-5 shadow rounded-sm">
        <h1 class="text-xl mb-3">Modules List</h1>
        <el-table
          :data="tableData"
          border
          style="width: 100%"
          ...
          v-loading="isLoading"
        >
          <el-table-column property="code" label="Module Code" show-overflow-tooltip> </el-table-column>
          <el-table-column property="name" label="Module Name" show-overflow-tooltip>
          </el-table-column>
          <el-table-column property="description" label="Description" show-overflow-tooltip>
          </el-table-column>
          <el-table-column width="170" property="creditHour" label="Credit Hour">
          </el-table-column>
        ...
        <div
          ...
          v-if="modules && modules.pager.totalItems > 0"
        >
          <div>
            <span>Showing</span>
            {{
              modules.pager.endIndex < modules.pager.totalItems
                ? modules.pager.endIndex
                : modules.pager.totalItems
            }}
            <span>of</span>
            {{ modules.pager.totalItems }}
          </div>
          <div>
            <pagination
              @change="changePage"
              :currentPage="modules.pager.currentPage"
              :pages="modules.pager.pages"
              :totalPages="modules.pager.totalPages"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    addNew() {
      return this.$store.commit("modules/setAdd", true);
    },
    changePage(page) {
      this.page = page;

      this.$store.dispatch("modules/proplateModules", {
        page,
        search: this.search,
      });
    },
    populateStore() {
      this.$store.dispatch("modules/find");
    },
  },
  mounted() {
    this.populateStore();
