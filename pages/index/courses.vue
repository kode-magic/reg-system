  ...
  <div>
    <div class="container mx-auto px-2 pt-2 pb-10">
      <div class="bg-white p-5 shadow rounded-sm">
        <h1 class="text-xl mb-3">Courses List</h1>
        <el-table
          :data="tableData"
          border
          stripe="true"
          ...
          v-loading="isLoading"
        >
          <el-table-column property="name" label="Name" show-overflow-tooltip> </el-table-column>
          <el-table-column property="description" label="Description" show-overflow-tooltip>
          </el-table-column>
          <el-table-column property="faculty" label="Faculty" show-overflow-tooltip>
          </el-table-column>
          <el-table-column width="125" property="totalStudents" label="Total Students">
          </el-table-column>
        ...
        <div
          ...
          v-if="courses && courses.pager.totalItems > 0"
        >
          <div>
            <span>Showing</span>
            {{
              courses.pager.endIndex < courses.pager.totalItems
                ? courses.pager.endIndex
                : courses.pager.totalItems
            }}
            <span>of</span>
            {{ courses.pager.totalItems }}
          </div>
          <div>
            <pagination
              @change="changePage"
              :currentPage="courses.pager.currentPage"
              :pages="courses.pager.pages"
              :totalPages="courses.pager.totalPages"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    addNew() {
      return this.$store.commit("courses/setAdd", true);
    },
    changePage(page) {
      this.page = page;

      this.$store.dispatch("courses/proplateCourses", {
        page,
        search: this.search,
      });
    },
